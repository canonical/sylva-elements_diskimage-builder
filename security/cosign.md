...
Export the environment variable `GITLAB_TOKEN` with rights to create CI/CD variables and push to the OCI registry.
...
Three variables are added to the project:
...
The manifest digest can be obtained via the API or by using crane:
...
```shell
docker login -u $GITLAB_USER -p $GITLAB_TOKEN registry.gitlab.com/sylva-projects/sylva-elements/diskimage-builder
```
